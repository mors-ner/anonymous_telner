...
def preprocess_data(X, y, num_unq_words, num_unq_tags):
    max_len = max([len(feature) for feature in X])
    print("Max Len is {}".format(max_len))

    X = pad_sequences(maxlen=max_len, sequences=X, padding="post", value=num_unq_words+1)
    y = pad_sequences(maxlen=max_len, sequences=y, padding="post", value=num_unq_tags+1)
    y = [to_categorical(i, num_classes=num_unq_tags+2) for i in y]
...
def get_trained_model(X_train, y_train, num_unq_words, num_unq_tags):
    ...
    model = Sequential()
    model.add(Embedding(
        input_dim=(num_unq_words + 2),
        output_dim=LSTMAttrs.EMBEDDING,
        input_length=max_len
    ))
    model.add(LSTM(
        units=LSTMAttrs.EMBEDDING,
        return_sequences=True,
        dropout=0.2,
        recurrent_dropout=0.2
    ))
    model.add(TimeDistributed(Dense(num_unq_tags+2, activation="softmax")))
...
def main():
    ...
    X, y = preprocess_data(X, y, num_unq_words, num_unq_tags)
    ...
    for i in range(len(y_test_true)):
        temp = np.delete(y_test_true[i], np.where(y_test_true[i] == num_unq_tags+1))
